...
(function (factory) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	function computeDurationGreatestUnit(duration, durationInput) {
		...
		if (unit === 'week' && (typeof durationInput === 'undefined' ? 'undefined' : _typeof(durationInput)) === 'object' && durationInput.days) {
			unit = 'day';
		}
 ...
 */
  ...
  */
	...
	var EmitterMixin = FC.EmitterMixin = {
		...
		hasHandlers: function hasHandlers(type) {
 ...
 */
  ...
  */
		applyProps: function applyProps(rawProps) {
  ...
  Meant to be only called ONCE, at object creation.
  */
		applyManualStandardProps: function applyManualStandardProps(rawProps) {
			return true;
		},
  ...
  */
		applyMiscProps: function applyMiscProps(rawProps) {
  ...
  */
		isStandardProp: function isStandardProp(propName) {
 ...
 */
	var ParsableModelMixin_defineStandardProps = function ParsableModelMixin_defineStandardProps(propDefs) {
 ...
 TODO: devise a better system
 */
	var ParsableModelMixin_copyVerbatimStandardProps = function ParsableModelMixin_copyVerbatimStandardProps(src, dest) {
	...
	var Model = Class.extend(EmitterMixin, ListenerMixin, {
		...
		setProps: function setProps(newProps) {
			...
			for (name in newProps) {
				...
				if ((typeof val === 'undefined' ? 'undefined' : _typeof(val)) === 'object' || val !== this._props[name]) {
	...
	var TaskQueue = Class.extend(EmitterMixin, {
		...
		queue: function queue() /* taskFunc, taskFunc... */{
		...
		runRemaining: function runRemaining() {
			// assumes at least one task in queue. does not check canRunNext for first task.
			var _this = this;
			var task;
			var res;
 ...
 */
	...
	var Interaction = Class.extend({
		...
		constructor: function constructor(component) {
  ...
  */
		...
		buildDragListener: function buildDragListener() {
			var _this = this;
			var component = this.component;
  ...
  */
		...
		computeSelection: function computeSelection(footprint0, footprint1) {
		...
		computeSelectionFootprint: function computeSelectionFootprint(footprint0, footprint1) {
			...
			return new ComponentFootprint(new UnzonedRange(ms[0], ms[3]), footprint0.isAllDay);
		},

		isSelectionFootprintAllowed: function isSelectionFootprintAllowed(componentFootprint) {
			return this.component.dateProfile.validUnzonedRange.containsRange(componentFootprint.unzonedRange) && this.view.calendar.isSelectionFootprintAllowed(componentFootprint);
		}
  ...
  */
		constructor: function constructor(component, eventPointing) {
			Interaction.call(this, component);
		...
		getSelectionDelay: function getSelectionDelay() {
		...
		handleMousedown: function handleMousedown(seg, ev) {
			if (this.component.canStartDrag(seg, ev)) {
				this.buildDragListener(seg).startInteraction(ev, { distance: 5 });
			}
		},

		handleTouchStart: function handleTouchStart(seg, ev) {
			var component = this.component;
			var settings = {
		...
		buildSelectListener: function buildSelectListener(seg) {
			var _this = this;
			var view = this.view;
		...
		// Has side effect of setting/unsetting `dragListener`
		buildDragListener: function buildDragListener(seg) {
			var _this = this;
			var component = this.component;
			var view = this.view;
		...
		segDragStart: function segDragStart(seg, ev) {
			...
			this.component.publiclyTrigger('eventDragStart', {
				context: seg.el[0],
				args: [seg.footprint.getEventLegacy(), ev, {}, // jqui dummy
				this.view]
			});
		},
		...
		computeEventDropMutation: function computeEventDropMutation(startFootprint, endFootprint, eventDef) {
		...
		computeEventDateMutation: function computeEventDateMutation(startFootprint, endFootprint) {
  ...
  */
		...
		// Generic enough to work with any type of Grid.
		buildDragListener: function buildDragListener(seg, isStart) {
			var _this = this;
			var component = this.component;
			var view = this.view;
			var calendar = view.calendar;
		...
		computeEventStartResizeMutation: function computeEventStartResizeMutation(startFootprint, endFootprint, origEventFootprint) {
		...
		computeEventEndResizeMutation: function computeEventEndResizeMutation(startFootprint, endFootprint, origEventFootprint) {
			var origRange = origEventFootprint.componentFootprint.unzonedRange;
  ...
  */
		...
		bindToDocument: function bindToDocument() {
			this.listenTo($(document), {
		...
		unbindFromDocument: function unbindFromDocument() {
			this.stopListeningTo($(document));
		},
			...
			var dragListener = _this.dragListener = new HitDragListener(component, {
				...
				hitOver: function hitOver(hit) {
					...
					if (hitFootprint) {
						...
						if (singleEventDef) {
							mutatedEventInstanceGroup = new EventInstanceGroup(singleEventDef.buildInstances());
		...
		// DOES NOT consider overlap/constraint.
		// Assumes both footprints are non-open-ended.
		computeExternalDrop: function computeExternalDrop(componentFootprint, meta) {
			...
			eventDef = SingleEventDef.parse($.extend({}, meta.eventProps, {
				start: start,
				end: end
  ...
  */
		...
		handleClick: function handleClick(seg, ev) {
		...
		handleMouseover: function handleMouseover(seg, ev) {
		...
		handleMouseout: function handleMouseout(seg, ev) {
			if (this.mousedOverSeg) {
				...
				this.component.publiclyTrigger('eventMouseout', {
					context: seg.el[0],
					args: [seg.footprint.getEventLegacy(), ev || {}, // if given no arg, make a mock mouse event
					this.view]
				});
			}
		},
	...
	var EventRenderer = FC.EventRenderer = Class.extend({
		...
		constructor: function constructor(component, fillRenderer) {
			// fillRenderer is optional
			this.view = component._getView();
			this.component = component;
		...
		render: function render(eventsPayload) {
		...
		renderFgRanges: function renderFgRanges(eventRanges) {
		...
		unrenderFgRanges: function unrenderFgRanges() {
			this.unrenderFgSegs(this.fgSegs || []);
			this.fgSegs = null;
		},

		renderBgRanges: function renderBgRanges(eventRanges) {
			var eventFootprints = this.component.eventRangesToEventFootprints(eventRanges);
			var segs = this.component.eventFootprintsToSegs(eventFootprints);
		...
		unrenderBgRanges: function unrenderBgRanges() {
			this.unrenderBgSegs();
			this.bgSegs = null;
		},

		getSegs: function getSegs() {
		...
		unrenderFgSegs: function unrenderFgSegs(segs) {
			// subclasses must implement
		},
		...
		beforeFgSegHtml: function beforeFgSegHtml(seg) {// hack
		},
		...
		filterEventRenderEl: function filterEventRenderEl(eventFootprint, el) {
		...
		getTimeText: function getTimeText(eventFootprint, formatStr, displayEnd) {
			return this._getTimeText(eventFootprint.eventInstance.dateProfile.start, eventFootprint.eventInstance.dateProfile.end, eventFootprint.componentFootprint.isAllDay, formatStr, displayEnd);
		},

		_getTimeText: function _getTimeText(start, end, isAllDay, formatStr, displayEnd) {
			if (formatStr == null) {
		...
		getBgClasses: function getBgClasses(eventDef) {
			var classNames = this.getClasses(eventDef);
			classNames.push('fc-bgevent');
			return classNames;
		},

		getClasses: function getClasses(eventDef) {
		...
		getSkinCss: function getSkinCss(eventDef) {
			return {
		...
		getBgColor: function getBgColor(eventDef) {
			var objs = this.getStylingObjs(eventDef);
			var i;
			var val;
		...
		// Queries for caller-specified color, then falls back to default
		getBorderColor: function getBorderColor(eventDef) {
			var objs = this.getStylingObjs(eventDef);
			var i;
			var val;
		...
		// Queries for caller-specified color, then falls back to default
		getTextColor: function getTextColor(eventDef) {
			var objs = this.getStylingObjs(eventDef);
			var i;
			var val;
		...
		getStylingObjs: function getStylingObjs(eventDef) {
			var objs = this.getFallbackStylingObjs(eventDef);
			objs.unshift(eventDef);
			return objs;
		},

		getFallbackStylingObjs: function getFallbackStylingObjs(eventDef) {
		...
		compareEventSegs: function compareEventSegs(seg1, seg2) {
			...
			compareByFieldSpecs(seg1.footprint.eventDef, seg2.footprint.eventDef, this.view.eventOrderSpecs);
		}
  ...
  */
		...
		render: function render(businessHourGenerator) {
			var component = this.component;
		...
		renderEventFootprints: function renderEventFootprints(eventFootprints) {
			var segs = this.component.eventFootprintsToSegs(eventFootprints);
		...
		renderSegs: function renderSegs(segs) {
			if (this.fillRenderer) {
	...
	var FillRenderer = FC.FillRenderer = Class.extend({ // use for highlight, background events, business hours
		...
		renderFootprint: function renderFootprint(type, componentFootprint, props) {
			this.renderSegs(type, this.component.componentFootprintToSegs(componentFootprint), props);
		},

		renderSegs: function renderSegs(type, segs, props) {
			var els;
		...
		unrender: function unrender(type) {
		...
		// Only returns segments that successfully rendered.
		buildSegEls: function buildSegEls(type, segs, props) {
			var _this = this;
			var html = '';
			var renderedSegs = [];
			var i;
		...
		buildSegHtml: function buildSegHtml(type, seg, props) {
		...
		attachSegEls: function attachSegEls(type, segs) {
			// subclasses must implement
		},

		reportEls: function reportEls(type, nodes) {
	...
	var HelperRenderer = FC.HelperRenderer = Class.extend({
		...
		constructor: function constructor(component, eventRenderer) {
			this.view = component._getView();
			this.component = component;
			this.eventRenderer = eventRenderer;
		},

		renderComponentFootprint: function renderComponentFootprint(componentFootprint) {
			this.renderEventFootprints([this.fabricateEventFootprint(componentFootprint)]);
		},

		renderEventDraggingFootprints: function renderEventDraggingFootprints(eventFootprints, sourceSeg, isTouch) {
			this.renderEventFootprints(eventFootprints, sourceSeg, 'fc-dragging', isTouch ? null : this.view.opt('dragOpacity'));
		},

		renderEventResizingFootprints: function renderEventResizingFootprints(eventFootprints, sourceSeg, isTouch) {
			this.renderEventFootprints(eventFootprints, sourceSeg, 'fc-resizing');
		},

		renderEventFootprints: function renderEventFootprints(eventFootprints, sourceSeg, extraClassNames, opacity) {
			var segs = this.component.eventFootprintsToSegs(eventFootprints);
  ...
  */
		renderSegs: function renderSegs(segs, sourceSeg) {
			// Subclasses must implement
		},
		...
		fabricateEventFootprint: function fabricateEventFootprint(componentFootprint) {
			var calendar = this.view.calendar;
  ...
  */
	...
	var DateComponent = FC.DateComponent = Component.extend({
		...
		updateSize: function updateSize(totalHeight, isAuto, isResize) {
		...
		publiclyTrigger: function publiclyTrigger() /**/{
			var calendar = this._getCalendar();

			return calendar.publiclyTrigger.apply(calendar, arguments);
		},

		hasPublicHandlers: function hasPublicHandlers() /**/{
			var calendar = this._getCalendar();
		...
		executeDateRender: function executeDateRender(dateProfile) {
		...
		renderDates: function renderDates(dateProfile) {
			// subclasses should implement
		},
		...
		renderBusinessHours: function renderBusinessHours(businessHourGenerator) {
		...
		executeEventRender: function executeEventRender(eventsPayload) {
		...
		executeEventUnrender: function executeEventUnrender() {
		...
		getBusinessHourSegs: function getBusinessHourSegs() {
			// recursive
		...
		getOwnBusinessHourSegs: function getOwnBusinessHourSegs() {
			if (this.businessHourRenderer) {
		...
		getOwnEventSegs: function getOwnEventSegs() {
			// just for itself
			if (this.eventRenderer) {
		...
		triggerAfterEventsRendered: function triggerAfterEventsRendered() {
		...
		triggerAfterEventSegsRendered: function triggerAfterEventSegsRendered(segs) {
			var _this = this;
		...
		triggerBeforeEventsDestroyed: function triggerBeforeEventsDestroyed() {
			this.triggerBeforeEventSegsDestroyed(this.getEventSegs());
		},

		triggerBeforeEventSegsDestroyed: function triggerBeforeEventSegsDestroyed(segs) {
			var _this = this;
		...
		showEventsWithId: function showEventsWithId(eventDefId) {
		...
		// RECURSIVE with subcomponents
		hideEventsWithId: function hideEventsWithId(eventDefId) {
		...
		renderDrag: function renderDrag(eventFootprints, seg, isTouch) {
		...
		renderEventResize: function renderEventResize(eventFootprints, seg, isTouch) {
		...
		renderSelectionFootprint: function renderSelectionFootprint(componentFootprint) {
		...
		renderHighlight: function renderHighlight(componentFootprint) {
			if (this.fillRenderer) {
		...
		getSafeHitFootprint: function getSafeHitFootprint(hit) {
		...
		getHitFootprint: function getHitFootprint(hit) {},
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		eventRangesToEventFootprints: function eventRangesToEventFootprints(eventRanges) {
			var eventFootprints = [];
			var i;
		...
		eventRangeToEventFootprints: function eventRangeToEventFootprints(eventRange) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		eventFootprintsToSegs: function eventFootprintsToSegs(eventFootprints) {
			var segs = [];
			var i;
		...
		eventFootprintToSegs: function eventFootprintToSegs(eventFootprint) {
		...
		componentFootprintToSegs: function componentFootprintToSegs(componentFootprint) {
			return [];
		},
		...
		callChildren: function callChildren(methodName, args) {
			this.iterChildren(function (child) {
				child[methodName].apply(child, args);
			});
		},

		iterChildren: function iterChildren(func) {
			var childrenByUid = this.childrenByUid;
			var uid;
		...
		_getCalendar: function _getCalendar() {
			// TODO: strip out. move to generic parent.
		...
		_getDateProfile: function _getDateProfile() {
	...
	DateComponent.mixin({
		...
		formatRange: function formatRange(range, isAllDay, formatStr, separator) {
			var end = range.end;
			...
			return _formatRange(range.start, end, formatStr, separator, this.isRTL);
		},
		...
		computeDayRange: function computeDayRange(unzonedRange) {
			var calendar = this._getCalendar();
		...
		isMultiDayRange: function isMultiDayRange(unzonedRange) {
	...
	var InteractiveDateComponent = FC.InteractiveDateComponent = DateComponent.extend({
		...
		bindDateHandlerToEl: function bindDateHandlerToEl(el, name, handler) {
			var _this = this;
		...
		bindAllSegHandlersToEl: function bindAllSegHandlersToEl(el) {
		...
		shouldIgnoreTouch: function shouldIgnoreTouch() {
			var view = this._getView();
		...
		shouldIgnoreEventPointing: function shouldIgnoreEventPointing() {
			// only call the handlers if there is not a drag/resize in progress
			return this.eventDragging && this.eventDragging.isDragging || this.eventResizing && this.eventResizing.isResizing;
		},

		canStartSelection: function canStartSelection(seg, ev) {
			return getEvIsTouch(ev) && !this.canStartResize(seg, ev) && (this.isEventDefDraggable(seg.footprint.eventDef) || this.isEventDefResizable(seg.footprint.eventDef));
		},

		canStartDrag: function canStartDrag(seg, ev) {
			return !this.canStartResize(seg, ev) && this.isEventDefDraggable(seg.footprint.eventDef);
		},

		canStartResize: function canStartResize(seg, ev) {
			var view = this._getView();
			var eventDef = seg.footprint.eventDef;

			return (!getEvIsTouch(ev) || view.isEventDefSelected(eventDef)) && this.isEventDefResizable(eventDef) && $(ev.target).is('.fc-resizer');
		},

		// Kills all in-progress dragging.
		// Useful for when public API methods that result in re-rendering are invoked during a drag.
		// Also useful for when touch devices misbehave and don't fire their touchend.
		endInteractions: function endInteractions() {
		...
		isEventDefDraggable: function isEventDefDraggable(eventDef) {
			return this.isEventDefStartEditable(eventDef);
		},

		isEventDefStartEditable: function isEventDefStartEditable(eventDef) {
		...
		isEventDefGenerallyEditable: function isEventDefGenerallyEditable(eventDef) {
		...
		isEventDefResizableFromStart: function isEventDefResizableFromStart(eventDef) {
			return this.opt('eventResizableFromStart') && this.isEventDefResizable(eventDef);
		},

		// Computes if the given event is allowed to be resized from its ending edge
		isEventDefResizableFromEnd: function isEventDefResizableFromEnd(eventDef) {
			return this.isEventDefResizable(eventDef);
		},

		// Computes if the given event is allowed to be resized by the user at all
		isEventDefResizable: function isEventDefResizable(eventDef) {
		...
		isEventInstanceGroupAllowed: function isEventInstanceGroupAllowed(eventInstanceGroup) {
			var view = this._getView();
		...
		isExternalInstanceGroupAllowed: function isExternalInstanceGroupAllowed(eventInstanceGroup) {
			var view = this._getView();
			var dateProfile = this.dateProfile;
			var eventFootprints = this.eventRangesToEventFootprints(eventInstanceGroup.getAllEventRanges());
			var i;
 ...
 */
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		sliceRangeByRow: function sliceRangeByRow(unzonedRange) {
		...
		sliceRangeByDay: function sliceRangeByDay(unzonedRange) {
			var daysPerRow = this.daysPerRow;
			var normalRange = this.view.computeDayRange(unzonedRange); // make whole-day range, considering nextDayThreshold
			var rangeFirst = this.getDateDayIndex(normalRange.start); // inclusive first index
			var rangeLast = this.getDateDayIndex(normalRange.end.clone().subtract(1, 'days')); // inclusive last index
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderHeadDateCellHtml: function renderHeadDateCellHtml(date, colspan, otherAttrs) {
			...
			return '' + '<th class="' + classNames.join(' ') + '"' + ((isDateValid && this.rowCnt) === 1 ? ' data-date="' + date.format('YYYY-MM-DD') + '"' : '') + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + (otherAttrs ? ' ' + otherAttrs : '') + '>' + (isDateValid ?
			// don't make a link if the heading could represent multiple days, or if there's only one day (forceOff)
			view.buildGotoAnchorHtml({ date: date, forceOff: this.rowCnt > 1 || this.colCnt === 1 }, innerHtml) :
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		initRenderQueue: function initRenderQueue() {
		...
		onRenderQueueStart: function onRenderQueueStart() {
			this.calendar.freezeContentHeight();
			this.addScroll(this.queryScroll());
		},

		onRenderQueueStop: function onRenderQueueStop() {
		...
		requestRender: function requestRender(func, namespace, actionType) {
			this.renderQueue.queue(func, namespace, actionType);
		},

		// given func will auto-bind to `this`
		whenSizeUpdated: function whenSizeUpdated(func) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeTitle: function computeTitle(dateProfile) {
			var unzonedRange;
		...
		computeTitleFormat: function computeTitleFormat(dateProfile) {
		...
		setEvents: function setEvents(eventsPayload) {
		...
		resetEvents: function resetEvents(eventsPayload) {
		...
		triggerViewRender: function triggerViewRender() {
		...
		triggerViewDestroy: function triggerViewDestroy() {
		...
		requestEventsRender: function requestEventsRender(eventsPayload) {
			var _this = this;
		...
		requestBusinessHoursRender: function requestBusinessHoursRender(businessHourGenerator) {
			var _this = this;
		...
		requestBusinessHoursUnrender: function requestBusinessHoursUnrender() {
			var _this = this;
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		reportEventDrop: function reportEventDrop(eventInstance, eventMutation, el, ev) {
		...
		triggerEventDrop: function triggerEventDrop(eventInstance, dateDelta, undoFunc, el, ev) {
			this.publiclyTrigger('eventDrop', {
				context: el[0],
				args: [eventInstance.toLegacy(), dateDelta, undoFunc, ev, {}, // {} = jqui dummy
				this]
			});
		},
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		reportExternalDrop: function reportExternalDrop(singleEventDef, isEvent, isSticky, el, ev, ui) {
		...
		triggerExternalDrop: function triggerExternalDrop(singleEventDef, isEvent, el, ev, ui) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		reportEventResize: function reportEventResize(eventInstance, eventMutation, el, ev) {
			var eventManager = this.calendar.eventManager;
			var undoFunc = eventManager.mutateEventsWithId(eventInstance.def.id, eventMutation, this.calendar);
		...
		triggerEventResize: function triggerEventResize(eventInstance, durationDelta, undoFunc, el, ev) {
			this.publiclyTrigger('eventResize', {
				context: el[0],
				args: [eventInstance.toLegacy(), durationDelta, undoFunc, ev, {}, // {} = jqui dummy
				this]
			});
		},
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		select: function select(footprint, ev) {
			this.unselect(ev);
			this.renderSelectionFootprint(footprint);
			this.reportSelection(footprint, ev);
		},

		renderSelectionFootprint: function renderSelectionFootprint(footprint, ev) {
		...
		reportSelection: function reportSelection(footprint, ev) {
			this.isSelected = true;
			this.triggerSelect(footprint, ev);
		},

		// Triggers handlers to 'select'
		triggerSelect: function triggerSelect(footprint, ev) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		selectEventInstance: function selectEventInstance(eventInstance) {
		...
		unselectEventInstance: function unselectEventInstance() {
		...
		isEventDefSelected: function isEventDefSelected(eventDef) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		triggerBaseRendered: function triggerBaseRendered() {
			this.publiclyTrigger('viewRender', {
				context: this,
				args: [this, this.el]
			});
		...
		triggerBaseUnrendered: function triggerBaseUnrendered() {
			this.publiclyTrigger('viewDestroy', {
				context: this,
				args: [this, this.el]
			});
		...
		triggerDayClick: function triggerDayClick(footprint, dayEl, ev) {
			var dateProfile = this.calendar.footprintToDateProfile(footprint); // abuse of "Event"DateProfile?
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		buildPrevDateProfile: function buildPrevDateProfile(date) {
			...
			var prevDate = date.clone().startOf(dateProfile.currentRangeUnit).subtract(dateProfile.dateIncrement);
		...
		buildNextDateProfile: function buildNextDateProfile(date) {
			var dateProfile = this.get('dateProfile');
			var nextDate = date.clone().startOf(dateProfile.currentRangeUnit).add(dateProfile.dateIncrement);
		...
		buildDateProfile: function buildDateProfile(date, direction, forceToValid) {
			...
			if (forceToValid) {
				date = this.calendar.msToUtcMoment(validUnzonedRange.constrainDate(date), // returns MS
				isDateAllDay);
			}
			...
			if (activeUnzonedRange) {
				date = this.calendar.msToUtcMoment(activeUnzonedRange.constrainDate(date), // returns MS
				isDateAllDay);
			}
		...
		buildValidRange: function buildValidRange() {
			return this.getUnzonedRangeOption('validRange', this.calendar.getNow()) || new UnzonedRange(); // completely open-ended
		},
		...
		adjustActiveRange: function adjustActiveRange(unzonedRange, minTime, maxTime) {
		...
		buildCustomVisibleRange: function buildCustomVisibleRange(date) {
			var visibleUnzonedRange = this.getUnzonedRangeOption('visibleRange', this.calendar.applyTimezone(date) // correct zone. also generates new obj that avoids mutations
			);
		...
		// not responsible for trimming hidden days.
		buildRenderRange: function buildRenderRange(currentUnzonedRange, currentRangeUnit, isRangeAllDay) {
		...
		trimHiddenDays: function trimHiddenDays(inputUnzonedRange) {
		...
		isDateInOtherMonth: function isDateInOtherMonth(date, dateProfile) {
			return false;
		},
		...
		getUnzonedRangeOption: function getUnzonedRangeOption(name) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	function Toolbar(calendar, toolbarOptions) {
					...
					$.each(this.split(','), function (j, buttonName) {
						...
						if (buttonName == 'title') {
							...
							if (customButtonProps = calendarCustomButtons[buttonName]) {
								...
								(buttonIcon = theme.getCustomButtonIconClass(customButtonProps)) || (buttonIcon = theme.getIconClass(buttonName)) || (buttonText = customButtonProps.text); // jshint ignore:line
							} else if (viewSpec = calendar.getViewSpec(buttonName)) {
								viewsWithButtons.push(buttonName);
								buttonClick = function buttonClick() {
									calendar.changeView(buttonName);
								};
								(buttonText = viewSpec.buttonTextOverride) || (buttonIcon = theme.getIconClass(buttonName)) || (buttonText = viewSpec.buttonTextDefault); // jshint ignore:line
							} else if (calendar[buttonName]) {
								// a calendar method
								buttonClick = function buttonClick() {
									calendar[buttonName]();
								};
								(buttonText = calendarButtonTextOverrides[buttonName]) || (buttonIcon = theme.getIconClass(buttonName)) || (buttonText = calendarButtonText[buttonName]); // jshint ignore:line
							...
							if (buttonClick) {
								...
								'<button type="button" class="' + buttonClasses.join(' ') + '">' + buttonInnerHtml + '</button>').click(function (ev) {
		...
		function activateButton(buttonName) {
			if (el) {
				el.find('.fc-' + buttonName + '-button').addClass(calendar.theme.getClass('stateActive'));
			}
		}

		function deactivateButton(buttonName) {
			if (el) {
				el.find('.fc-' + buttonName + '-button').removeClass(calendar.theme.getClass('stateActive'));
			}
		}

		function disableButton(buttonName) {
			if (el) {
				el.find('.fc-' + buttonName + '-button').prop('disabled', true).addClass(calendar.theme.getClass('stateDisabled'));
			}
		}

		function enableButton(buttonName) {
			if (el) {
				el.find('.fc-' + buttonName + '-button').prop('disabled', false).removeClass(calendar.theme.getClass('stateDisabled'));
			}
		}
	...
	var Calendar = FC.Calendar = Class.extend(EmitterMixin, ListenerMixin, {
		...
		publiclyTrigger: function publiclyTrigger(name, triggerInfo) {
		...
		hasPublicHandlers: function hasPublicHandlers(name) {
		...
		buildSelectFootprint: function buildSelectFootprint(zonedStartInput, zonedEndInput) {
			...
			return new ComponentFootprint(new UnzonedRange(start, end), !start.hasTime());
		},
		...
		parseUnzonedRange: function parseUnzonedRange(rangeInput) {
			var start = null;
			var end = null;
		...
		initEventManager: function initEventManager() {
			var _this = this;
		...
		requestEvents: function requestEvents(start, end) {
 ...
 */
	...
	Calendar.mixin({
		...
		msToMoment: function msToMoment(ms, forceAllDay) {
		...
		msToUtcMoment: function msToUtcMoment(ms, forceAllDay) {
			var mom = FC.moment.utc(ms); // TODO: optimize by using Date.UTC
  ...
  */
		footprintToDateProfile: function footprintToDateProfile(componentFootprint, ignoreEnd) {
	...
	Calendar.mixin({
		...
		bindViewHandlers: function bindViewHandlers(view) {
			var _this = this;
		...
		unbindViewHandlers: function unbindViewHandlers(view) {
		...
		renderView: function renderView(viewType) {
		...
		updateViewSize: function updateViewSize(isResize) {
			var view = this.view;
			var scroll;
		...
		windowResize: function windowResize(ev) {
			...
			this.view && this.view.isDatesRendered) {
  ...
  -----------------------------------------------------------------------------*/
		...
		forceFreezeContentHeight: function forceFreezeContentHeight() {
			this.contentEl.css({
				width: '100%',
	...
	Calendar.mixin({
		...
		updateToolbarButtons: function updateToolbarButtons(dateProfile) {
			var now = this.getNow();
			var view = this.view;
 ...
 */
	Calendar.prototype.isEventInstanceGroupAllowed = function (eventInstanceGroup) {
		...
		if (eventAllowFunc) {
			for (i = 0; i < eventFootprints.length; i++) {
				if (eventAllowFunc(eventFootprints[i].componentFootprint.toLegacy(this), eventFootprints[i].getEventLegacy()) === false) {
					return false;
				}
			}
		}
	...
	Calendar.prototype.isFootprintAllowed = function (componentFootprint, peerEventFootprints, constraintVal, overlapVal, subjectEventInstance // optional
	) {
	...
	Calendar.prototype.constraintValToFootprints = function (constraintVal, isAllDay) {
		...
		} else if ((typeof constraintVal === 'undefined' ? 'undefined' : _typeof(constraintVal)) === 'object') {
 ...
 */
	Calendar.prototype.parseFootprints = function (rawInput) {
		...
		return [new ComponentFootprint(new UnzonedRange(start, end), start && !start.hasTime() || end && !end.hasTime() // isAllDay
		)];
	};
	...
	Calendar.mixin({
		...
		getEventSourceById: function getEventSourceById(id) {
			return this.eventManager.getSourceById(EventSource.normalizeId(id));
		},

		addEventSource: function addEventSource(sourceInput) {
		...
		removeEventSources: function removeEventSources(sourceMultiQuery) {
			var eventManager = this.eventManager;
			var sources;
			var i;
		...
		removeEventSource: function removeEventSource(sourceQuery) {
			var eventManager = this.eventManager;
		...
		refetchEventSources: function refetchEventSources(sourceMultiQuery) {
			var eventManager = this.eventManager;
		...
		renderEvents: function renderEvents(eventInputs, isSticky) {
		...
		renderEvent: function renderEvent(eventInput, isSticky) {
			var eventManager = this.eventManager;
			var eventDef = EventDefParser.parse(eventInput, eventInput.source || eventManager.stickySource);
		...
		removeEvents: function removeEvents(legacyQuery) {
			var eventManager = this.eventManager;
			var legacyInstances = [];
			var idMap = {};
			var eventDef;
		...
		// legacyQuery operates on legacy event instance objects
		clientEvents: function clientEvents(legacyQuery) {
		...
		updateEvents: function updateEvents(eventPropsArray) {
			this.eventManager.freeze();
		...
		updateEvent: function updateEvent(eventProps) {
			...
			if (eventDef instanceof SingleEventDef) {
				...
				eventDefMutation = EventDefMutation.createFromRawProps(eventInstance, eventProps, // raw props
	...
	var UnzonedRange = FC.UnzonedRange = Class.extend({
		...
		constructor: function constructor(startInput, endInput) {
		...
		intersect: function intersect(otherRange) {
		...
		intersectsWith: function intersectsWith(otherRange) {
		...
		containsRange: function containsRange(innerRange) {
		...
		containsDate: function containsDate(date) {
			var ms = date.valueOf();
		...
		equals: function equals(otherRange) {
 ...
 */
	var ComponentFootprint = FC.ComponentFootprint = Class.extend({
		...
		constructor: function constructor(unzonedRange, isAllDay) {
  ...
  Only works for non-open-ended ranges.
  */
		toLegacy: function toLegacy(calendar) {
			return {
	...
	var EventPeriod = Class.extend(EmitterMixin, {
		...
		constructor: function constructor(start, end, timezone) {
			this.start = start;
			this.end = end;
			this.timezone = timezone;

			this.unzonedRange = new UnzonedRange(start.clone().stripZone(), end.clone().stripZone());
		...
		isWithinRange: function isWithinRange(start, end) {
		...
		requestSources: function requestSources(sources) {
			this.freeze();
		...
		requestSource: function requestSource(source) {
			var _this = this;
		...
		purgeSource: function purgeSource(source) {
		...
		purgeAllSources: function purgeAllSources() {
		...
		getEventDefByUid: function getEventDefByUid(eventDefUid) {
			return this.eventDefsByUid[eventDefUid];
		},

		getEventDefsById: function getEventDefsById(eventDefId) {
		...
		addEventDefs: function addEventDefs(eventDefs) {
			for (var i = 0; i < eventDefs.length; i++) {
				this.addEventDef(eventDefs[i]);
			}
		},

		addEventDef: function addEventDef(eventDef) {
		...
		removeEventDefsById: function removeEventDefsById(eventDefId) {
			var _this = this;
		...
		removeAllEventDefs: function removeAllEventDefs() {
		...
		removeEventDef: function removeEventDef(eventDef) {
			var eventDefsById = this.eventDefsById;
		...
		getEventInstances: function getEventInstances() {
		...
		getEventInstancesWithId: function getEventInstancesWithId(eventDefId) {
		...
		getEventInstancesWithoutId: function getEventInstancesWithoutId(eventDefId) {
			// TODO: consider iterator
			var eventInstanceGroupsById = this.eventInstanceGroupsById;
		...
		addEventInstance: function addEventInstance(eventInstance, eventDefId) {
			var eventInstanceGroupsById = this.eventInstanceGroupsById;
			var eventInstanceGroup = eventInstanceGroupsById[eventDefId] || (eventInstanceGroupsById[eventDefId] = new EventInstanceGroup());
		...
		removeEventInstancesForDef: function removeEventInstancesForDef(eventDef) {
			var eventInstanceGroupsById = this.eventInstanceGroupsById;
		...
		tryRelease: function tryRelease() {
			if (!this.pendingCnt) {
				if (!this.freezeDepth) {
					this.release();
				} else {
	...
	var EventManager = Class.extend(EmitterMixin, ListenerMixin, {
		...
		constructor: function constructor(calendar) {
			this.calendar = calendar;
			this.stickySource = new ArrayEventSource(calendar);
			this.otherSources = [];
		},

		requestEvents: function requestEvents(start, end, timezone, force) {
		...
		addSource: function addSource(eventSource) {
		...
		removeSource: function removeSource(doomedSource) {
		...
		removeAllSources: function removeAllSources() {
			this.otherSources = [];
		...
		refetchSource: function refetchSource(eventSource) {
		...
		refetchAllSources: function refetchAllSources() {
			var currentPeriod = this.currentPeriod;
		...
		multiQuerySources: function multiQuerySources(matchInputs) {
		...
		querySources: function querySources(matchInput) {
  ...
  */
		getSourceById: function getSourceById(id) {
		...
		setPeriod: function setPeriod(eventPeriod) {
			if (this.currentPeriod) {
		...
		bindPeriod: function bindPeriod(eventPeriod) {
			this.listenTo(eventPeriod, 'release', function (eventsPayload) {
				this.trigger('release', eventsPayload);
			});
		},

		unbindPeriod: function unbindPeriod(eventPeriod) {
		...
		getEventDefByUid: function getEventDefByUid(uid) {
			if (this.currentPeriod) {
				return this.currentPeriod.getEventDefByUid(uid);
			}
		},

		addEventDef: function addEventDef(eventDef, isSticky) {
			if (isSticky) {
		...
		removeEventDefsById: function removeEventDefsById(eventId) {
  ...
  */
		mutateEventsWithId: function mutateEventsWithId(eventDefId, eventDefMutation) {
			var currentPeriod = this.currentPeriod;
			var eventDefs;
			var undoFuncs = [];
  ...
  */
		buildMutatedEventInstanceGroup: function buildMutatedEventInstanceGroup(eventDefId, eventDefMutation) {
	...
	['getEventDefsById', 'getEventInstances', 'getEventInstancesWithId', 'getEventInstancesWithoutId'].forEach(function (methodName) {
	...
	var BusinessHourGenerator = FC.BusinessHourGenerator = Class.extend({
		...
		constructor: function constructor(rawComplexDef, calendar) {
			this.rawComplexDef = rawComplexDef;
			this.calendar = calendar;
		},

		buildEventInstanceGroup: function buildEventInstanceGroup(isAllDay, unzonedRange) {
			var eventDefs = this.buildEventDefs(isAllDay);
			var eventInstanceGroup;

			if (eventDefs.length) {
				eventInstanceGroup = new EventInstanceGroup(eventDefsToEventInstances(eventDefs, unzonedRange));
		...
		buildEventDefs: function buildEventDefs(isAllDay) {
		...
		buildEventDef: function buildEventDef(isAllDay, rawDef) {
			...
			return RecurringEventDef.parse(fullRawDef, new EventSource(this.calendar) // dummy source
			);
		}
	...
	var EventDefParser = {
		...
		parse: function parse(eventInput, source) {
			if (isTimeString(eventInput.start) || moment.isDuration(eventInput.start) || isTimeString(eventInput.end) || moment.isDuration(eventInput.end)) {
	...
	var EventDef = FC.EventDef = Class.extend(ParsableModelMixin, {
		...
		buildInstances: function buildInstances(unzonedRange) {
			// subclasses must implement
		},
		...
		hasInverseRendering: function hasInverseRendering() {
		...
		isStartExplicitlyEditable: function isStartExplicitlyEditable() {
		...
		isDurationExplicitlyEditable: function isDurationExplicitlyEditable() {
		...
		isExplicitlyEditable: function isExplicitlyEditable() {
  ...
  */
		buildInstances: function buildInstances() {
			return [this.buildInstance()];
		},

		buildInstance: function buildInstance() {
			return new EventInstance(this, // definition
			this.dateProfile);
		},
		...
		rezone: function rezone() {
			var calendar = this.source.calendar;
			var dateProfile = this.dateProfile;

			this.dateProfile = new EventDateProfile(calendar.moment(dateProfile.start), dateProfile.end ? calendar.moment(dateProfile.end) : null, calendar);
		},
  ...
  */
	...
	var RecurringEventDef = EventDef.extend({
		...
		buildInstances: function buildInstances(unzonedRange) {
			...
			while (unzonedDate.isBefore(unzonedEnd)) {
				...
				if (!this.dowHash || this.dowHash[unzonedDate.day()]) {
					...
					instances.push(new EventInstance(this, // definition
		...
		setDow: function setDow(dowNumbers) {
  ...
  */
	...
	var EventInstance = Class.extend({
		...
		constructor: function constructor(def, dateProfile) {
			this.def = def;
			this.dateProfile = dateProfile;
		},
 ...
 */
	var EventInstanceGroup = FC.EventInstanceGroup = Class.extend({
		...
		constructor: function constructor(eventInstances) {
			this.eventInstances = eventInstances || [];
		},

		getAllEventRanges: function getAllEventRanges(constraintRange) {
			if (constraintRange) {
		...
		sliceRenderRanges: function sliceRenderRanges(constraintRange) {
			if (this.isInverse()) {
		...
		sliceNormalRenderRanges: function sliceNormalRenderRanges(constraintRange) {
			...
			for (i = 0; i < eventInstances.length; i++) {
				...
				if (slicedRange) {
					slicedEventRanges.push(new EventRange(slicedRange, eventInstance.def, eventInstance));
				}
			}
		...
		sliceInverseRenderRanges: function sliceInverseRenderRanges(constraintRange) {
		...
		getEventDef: function getEventDef() {
 ...
 */
	var EventDateProfile = Class.extend({
		...
		constructor: function constructor(start, end, calendar) {
			this.start = start;
			this.end = end || null;
  ...
  */
		buildUnzonedRange: function buildUnzonedRange(calendar) {
  ...
  Needs a Calendar object
  */
		getEnd: function getEnd(calendar) {
 ...
 */
	...
	var EventRange = Class.extend({
		...
		constructor: function constructor(unzonedRange, eventDef, eventInstance) {
			this.unzonedRange = unzonedRange;
	...
	var EventFootprint = FC.EventFootprint = Class.extend({
		...
		constructor: function constructor(componentFootprint, eventDef, eventInstance) {
		...
		getEventLegacy: function getEventLegacy() {
  ...
  returns an undo function.
  */
		mutateSingle: function mutateSingle(eventDef) {
			var origDateProfile;
			...
			if (this.verbatimStandardProps) {
				SingleEventDef.copyVerbatimStandardProps(this.verbatimStandardProps, // src
				eventDef // dest
				);
			}
		...
		setDateMutation: function setDateMutation(dateMutation) {
	...
	EventDefMutation.createFromRawProps = function (eventInstance, rawProps, largeUnit) {
		...
		if (dateProfile) {
			// no failure?
			dateMutation = EventDefDateMutation.createFromDiff(eventInstance.dateProfile, dateProfile, largeUnit);
		}
		...
		EventDef.copyVerbatimStandardProps(standardProps, // src
  ...
  returns an undo function.
  */
		buildNewDateProfile: function buildNewDateProfile(eventDateProfile, calendar) {
		...
		setDateDelta: function setDateDelta(dateDelta) {
		...
		setStartDelta: function setStartDelta(startDelta) {
		...
		setEndDelta: function setEndDelta(endDelta) {
	...
	function eventInstanceToEventRange(eventInstance) {
		return new EventRange(eventInstance.dateProfile.unzonedRange, eventInstance.def, eventInstance);
	}

	function eventRangeToEventFootprint(eventRange) {
		return new EventFootprint(new ComponentFootprint(eventRange.unzonedRange, eventRange.eventDef.isAllDay()), eventRange.eventDef, eventRange.eventInstance // might not exist
		);
	}
	...
	var EventSource = Class.extend(ParsableModelMixin, {
		...
		fetch: function fetch(start, end, timezone) {
  ...
  */
		getPrimitive: function getPrimitive(otherSource) {
			// subclasses must implement
		},

		parseEventDefs: function parseEventDefs(rawEventDefs) {
			var i;
			var eventDef;
			var eventDefs = [];

			for (i = 0; i < rawEventDefs.length; i++) {
				eventDef = EventDefParser.parse(rawEventDefs[i], this // source
				);
 ...
 */
	EventSource.parse = function (rawInput, calendar) {
		...
		if ((typeof rawInput === 'undefined' ? 'undefined' : _typeof(rawInput)) === 'object') {
	...
	var ArrayEventSource = EventSource.extend({
		...
		setRawEventDefs: function setRawEventDefs(rawEventDefs) {
  ...
  */
	...
	var JsonFeedEventSource = EventSource.extend({
		...
		fetch: function fetch(start, end, timezone) {
			...
			return Promise.construct(function (onResolve, onReject) {
				...
				JsonFeedEventSource.AJAX_DEFAULTS, ajaxSettings, {
					url: _this.url,
					data: requestParams,
					success: function success(rawEventDefs) {
						var callbackRes;
		...
		buildRequestParams: function buildRequestParams(start, end, timezone) {
			var calendar = this.calendar;
			var ajaxSettings = this.ajaxSettings;
	...
	var ThemeRegistry = FC.ThemeRegistry = {
		...
		register: function register(themeName, themeClass) {
			this.themeClassHash[themeName] = themeClass;
		},

		getThemeClass: function getThemeClass(themeSetting) {
			if (!themeSetting) {
	...
	var Theme = FC.Theme = Class.extend({
		...
		constructor: function constructor(optionsModel) {
			this.optionsModel = optionsModel;
			this.processIconOverride();
		},

		processIconOverride: function processIconOverride() {
			if (this.iconOverrideOption) {
				this.setIconOverride(this.optionsModel.get(this.iconOverrideOption));
			}
		},

		setIconOverride: function setIconOverride(iconOverrideHash) {
			var iconClassesCopy;
			var buttonName;
		...
		applyIconOverridePrefix: function applyIconOverridePrefix(className) {
		...
		getIconClass: function getIconClass(buttonName) {
		...
		getCustomButtonIconClass: function getCustomButtonIconClass(customButtonProps) {
			var className;
	...
	var DayGridFillRenderer = FillRenderer.extend({
		...
		renderFillRow: function renderFillRow(type, seg) {
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	var DayGridEventRenderer = EventRenderer.extend({
		...
		constructor: function constructor(dayGrid) {
		...
		fgSegHtml: function fgSegHtml(seg, disableResizing) {
			...
			return '<a class="' + classes.join(' ') + '"' + (eventDef.url ? ' href="' + htmlEscape(eventDef.url) + '"' : '') + (skinCss ? ' style="' + skinCss + '"' : '') + '>' + '<div class="fc-content">' + (this.isRTL ? titleHtml + ' ' + timeHtml : // put a natural space in between
			timeHtml + ' ' + titleHtml //
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		getIsNumbersVisible: function getIsNumbersVisible() {
			return this.getIsDayNumbersVisible() || this.cellWeekNumbersVisible;
		},

		getIsDayNumbersVisible: function getIsDayNumbersVisible() {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		getHitFootprint: function getHitFootprint(hit) {
			...
			return new ComponentFootprint(new UnzonedRange(range.start, range.end), true // all-day?
			);
		},
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		unrenderDrag: function unrenderDrag(seg) {
			this.unrenderHighlight();
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		// Unrenders a visual indication of an event being resized
		unrenderEventResize: function unrenderEventResize(seg) {
			this.unrenderHighlight();
			this.helperRenderer.unrender();
		}
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
	...
	DayGrid.mixin({
		...
		renderSegPopoverContent: function renderSegPopoverContent(row, col, segs) {
			...
			var content = $('<div class="fc-header ' + theme.getClass('popoverHeader') + '">' + '<span class="fc-close ' + theme.getIconClass('close') + '"></span>' + '<span class="fc-title">' + htmlEscape(title) + '</span>' + '<div class="fc-clear"/>' + '</div>' + '<div class="fc-body ' + theme.getClass('popoverContent') + '">' + '<div class="fc-event-container"></div>' + '</div>');
			...
			for (i = 0; i < segs.length; i++) {
				...
				if (slicedRange) {
					newSegs.push($.extend({}, seg, {
						footprint: new EventFootprint(new ComponentFootprint(slicedRange, seg.footprint.componentFootprint.isAllDay), seg.footprint.eventDef, seg.footprint.eventInstance),
 ...
 */
	var TimeGridEventRenderer = EventRenderer.extend({
		...
		constructor: function constructor(timeGrid) {
			EventRenderer.apply(this, arguments);
		...
		fgSegHtml: function fgSegHtml(seg, disableResizing) {
			...
			return '<a class="' + classes.join(' ') + '"' + (eventDef.url ? ' href="' + htmlEscape(eventDef.url) + '"' : '') + (skinCss ? ' style="' + skinCss + '"' : '') + '>' + '<div class="fc-content">' + (timeText ? '<div class="fc-time"' + ' data-start="' + htmlEscape(startTimeText) + '"' + ' data-full="' + htmlEscape(fullTimeText) + '"' + '>' + '<span>' + htmlEscape(timeText) + '</span>' + '</div>' : '') + (eventDef.title ? '<div class="fc-title">' + htmlEscape(eventDef.title) + '</div>' : '') + '</div>' + '<div class="fc-bg"/>' + (
 ...
 ----------------------------------------------------------------------------------------------------------------------*/
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		sliceRangeByTimes: function sliceRangeByTimes(unzonedRange) {
			var segs = [];
			var segRange;
			var dayIndex;
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderSlats: function renderSlats() {
			var theme = this.view.calendar.theme;
		...
		renderColumns: function renderColumns() {
			...
			this.dayRanges = this.dayDates.map(function (dayDate) {
				return new UnzonedRange(dayDate.clone().add(dateProfile.minTime), dayDate.clone().add(dateProfile.maxTime));
			});
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		renderNowIndicator: function renderNowIndicator(date) {
			...
			var segs = this.componentFootprintToSegs(new ComponentFootprint(new UnzonedRange(date, date.valueOf() + 1), // protect against null range
			false // all-day
			));
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		computeDateTop: function computeDateTop(ms, startOfDayDate) {
  ...
  ------------------------------------------------------------------------------------------------------------------*/
		...
		getHitFootprint: function getHitFootprint(hit) {
			...
			return new ComponentFootprint(new UnzonedRange(start, end), false // all-day?
			);
		},
	...
	var agendaTimeGridMethods = {
		...
		renderHeadIntroHtml: function renderHeadIntroHtml() {
			...
			if (this.opt('weekNumbers')) {
				...
				{ date: weekStart, type: 'week', forceOff: this.colCnt > 1 }, htmlEscape(weekText) // inner HTML
				) + '</th>';
			} else {
	...
	var ListView = FC.ListView = View.extend({
		...
		renderDates: function renderDates(dateProfile) {
			...
			while (dayStart < viewEnd) {
				...
				dayRanges.push(new UnzonedRange(dayStart, dayStart.clone().add(1, 'day')));
		...
		// slices by day
		componentFootprintToSegs: function componentFootprintToSegs(footprint) {
			...
			fgSegHtml: function fgSegHtml(seg) {
				...
				// if the event appears to span more than one day
						...
						if (seg.isStart || seg.isEnd) {
							...
							timeHtml = htmlEscape(this._getTimeText(calendar.msToMoment(seg.startMs), calendar.msToMoment(seg.endMs), componentFootprint.isAllDay));
						} else {
		...
		dayHeaderHtml: function dayHeaderHtml(dayDate) {
			...
			return '<tr class="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '">' + '<td class="' + this.calendar.theme.getClass('widgetHeader') + '" colspan="3">' + (mainFormat ? this.buildGotoAnchorHtml(dayDate, { 'class': 'fc-list-heading-main' }, htmlEscape(dayDate.format(mainFormat)) // inner HTML
			) : '') + (altFormat ? this.buildGotoAnchorHtml(dayDate, { 'class': 'fc-list-heading-alt' }, htmlEscape(dayDate.format(altFormat)) // inner HTML
			) : '') + '</td>' + '</tr>';
		}
