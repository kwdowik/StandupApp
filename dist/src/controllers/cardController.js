...
var cardController = function cardController(nav) {

    var getNotePage = function getNotePage(req, res) {
        ...
        dbService.getUsers().then(function (users) {
            query.sort({ createdOn: 'desc' }).skip((req.params.cardNumber - 1) * 5).limit(5).exec(function (err, results) {
                res.render('index', {
                    title: 'Standup - List',
                    notes: results,
    ...
    var getNumberOfNotes = function getNumberOfNotes(filter) {
        var query = Note.find();
    ...
    var getNoteByMember = function getNoteByMember(req, res) {
        var query = Note.find();
        ...
        if (filter.length > 0) {
            query.where({ memberName: filter }).limit(5);
        } else {
            query.skip((req.params.cardNumber - 1) * 5).limit(5);
        }
